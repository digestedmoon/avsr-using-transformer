...
def trgtlen_distribution_checker():
    for dataset in ["pretrain","main"]:
        if dataset == "pretrain":
            distribution = np.zeros(2500, dtype=int)
        else:
            distribution = np.zeros(150, dtype=int)
...
def word_length_distribution_checker():
    distribution = np.zeros(35, dtype=int)
...
def word_count_distribution_checker():
    for dataset in ["pretrain","main"]:
        if dataset == "pretrain":
            distribution = np.zeros(500, dtype=int)
        else:
            distribution = np.zeros(30, dtype=int)
        for root, dirs, files in os.walk(args["DATA_DIRECTORY"] + "/" + dataset):
            for file in files:
                if file.endswith(".mp4"):
                    targetFile = os.path.join(root, file[:-4]) + ".txt"
